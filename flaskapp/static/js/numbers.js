...
function compute() {
    ...
    $.ajax({
        url: '/countdown/api/v1.0/numbers',
        data: request_json,
        success: async function (data, textStatus, jqXHR) {
            if (data['result'][0] === "It's Impossible!") {
                ...
                html =
                    '<h1 class="display-6 animated fadeIn">Solution: </h1>\n<ul class="list-group" id="solution-list">\n';
                html += '</ul>\n';
                $('#solution').html(html);
                for (var x = 0; x < data['result'].length; x++) {
                    $('#solution-list').append(
                        '<li class="list-group-item animated fadeIn">' + data['result'][x] + '</li>'
                    );
            ...
            $('#solution').append(
                '<br /><a style="float:right;" class="btn btn-primary btn-lg" href="numbers.html">Reset</a>'
            );
            $('#computeBtn').removeClass('disabled');

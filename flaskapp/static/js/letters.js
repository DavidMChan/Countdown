...
function compute() {
    ...
    $.ajax({
        ...
        success:async function(data, textStatus, jqXHR) {
                ...
                for (var x = 0; data['result'][x].length == 9 ||  x < Math.min(data['result'].length, 9); x++) {
                    if (data['result'][x].length == 9) {
                        $("#solution-list").append(
                            ...
                            "<span class=\"badge badge-pill badge-danger\">" + data['result'][x].length + "</span>" +
                            "<span style=\"padding-left: 30px\">" + data['result'][x] + "</span>" +
                            "</li>"
                            );
                    } else if (data['result'][x].length > 6) {
                        $("#solution-list").append(
                            "<li class=\"list-group-item animated fadeIn\">" +
                            "<span class=\"badge badge-pill badge-warning\">" + data['result'][x].length + "</span>" +
                            "<span style=\"padding-left: 30px\">" + data['result'][x] + "</span>" +
                            "</li>"
                            );
                        ...
                        $("#solution-list").append(
                            "<li class=\"list-group-item animated fadeIn\">" +
                            "<span class=\"badge badge-pill badge-success\">" + data['result'][x].length + "</span>" +
                            "<span style=\"padding-left: 30px\">" + data['result'][x] + "</span>" +
                            "</li>"
                            );
                    } else {
                        $("#solution-list").append(
                            "<li class=\"list-group-item animated fadeIn\">" +
                            "<span class=\"badge badge-pill badge-primary\">" + data['result'][x].length + "</span>" +
                            "<span style=\"padding-left: 30px\">" + data['result'][x] + "</span>" +
                            "</li>"
                            );
                    }  
            ...
            $('#solution').append("<br /><a style=\"float:right;\" class=\"btn btn-primary btn-lg\" href=\"letters.html\">Reset</a>")
            $('#computeBtn').removeClass('disabled');
...
function compute_con() {
    ...
    $.ajax({
        ...
        success:async function(data, textStatus, jqXHR) {
            ...
            $('#solution').append("<br /><a style=\"float:right;\" class=\"btn btn-primary btn-lg\" href=\"conundrum.html\">Reset</a>")
            $('#computeBtn').removeClass('disabled');
            $('#computeBtn').html(original_html);
